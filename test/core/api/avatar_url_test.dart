...
void main() {
  test('resolves the relative worker route', () {
    ...
    check(resolveAvatarUrl('/avatars/u1?v=123', _base))
  ...
  test('passes an absolute bucket URL through untouched', () {
    ...
    check(resolveAvatarUrl('https://cdn.example.com/u1?v=123', _base))
  ...
  test('preserves the cache-buster', () {
    ...
    check(resolveAvatarUrl('/avatars/u1?v=999', _base)).isNotNull().endsWith('?v=999');
  });

  test('tolerates a base with a trailing slash', () {
    check(resolveAvatarUrl('/avatars/u1?v=1', 'https://api.example.com/'))
